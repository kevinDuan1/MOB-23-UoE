...
class DatasetHandler:
    ...
    def __init__(self):
        ...
        self.image_dir = os.path.join(root_dir_path, 'vo/image')
        self.depth_dir = os.path.join(root_dir_path, 'vo/depth')
    ...
    def _read_image(self):
        for i in range(1, self.num_frames + 1):
            ...
            im_name = "{0}/img{1}_0.png".format(self.image_dir, str(i+1249))
    ...
    def _read_depth(self):
        for i in range(1, self.num_frames + 1):
            ...
            depth_name = "{0}/img{1}_0.depth".format(self.depth_dir, str(i+1249))
            with open(depth_name, "r") as f:     
                data = (f.read())
            data = data.split(' ')[:-1]
            data = [float(x)  for x in data]
            data = [1000 if x > 1000 else x for x in data]
            depth = np.reshape(data,(480,640))
